...
import { AppNotificationSeverity, StoreState } from "types";
...
class UnConnectedAlertTab extends PureComponent<Props, State> {
  ...
  async componentDidMount() {
    // no angular support
    if (/*config.angularSupportEnabled*/ false) {
      await import(
        /* webpackChunkName: "AlertTabCtrl" */ "features/alerting/AlertTabCtrl"
      );
      this.loadAlertTab();
    } else {
...
const mapStateToProps: MapStateToProps<ConnectedProps, OwnProps, StoreState> = (
  state,
  props
) => {
  return {
    angularPanelComponent: getPanelStateForModel(state, props.panel)
      ?.angularComponent,
  };
};
